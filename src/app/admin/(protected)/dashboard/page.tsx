...
export default function AdminDashboard() {
  ...
  const handleDelete = async (id: string, title: string) => {
    ...
    const confirmDelete = window.confirm(`Apakah Anda yakin ingin menghapus "${title}" secara permanen?`)
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 text-sm">Kelola semua konten foto dan video portofolio Anda.</p>
        </div>
        <div>
          <Link
            href="/admin/portfolio/create"
            className="inline-flex items-center gap-2 px-5 py-3 rounded-xl text-sm font-semibold bg-gradient-to-r from-[#FF204E] to-[#A0153E] text-white shadow-lg shadow-[#FF204E]/25 hover:opacity-90 hover:scale-105 active:scale-95 transition-all cursor-pointer"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Card 1: Total Karya */}
        <div className="p-6 rounded-2xl bg-[#5D0E41]/10 border border-white/5 space-y-2">
          <p className="text-xs font-semibold uppercase text-gray-400 tracking-wider">Total Karya</p>
          <p className="text-3xl sm:text-4xl font-extrabold text-[#FF204E]">{totalItems}</p>
        </div>
        {/* Card 2: Total Foto */}
        <div className="p-6 rounded-2xl bg-[#5D0E41]/10 border border-white/5 space-y-2">
          <p className="text-xs font-semibold uppercase text-gray-400 tracking-wider">Total Foto</p>
          <p className="text-3xl sm:text-4xl font-extrabold text-[#FF204E]">{totalPhotos}</p>
        </div>
        {/* Card 3: Total Video */}
        <div className="p-6 rounded-2xl bg-[#5D0E41]/10 border border-white/5 space-y-2">
          <p className="text-xs font-semibold uppercase text-gray-400 tracking-wider">Total Video</p>
          <p className="text-3xl sm:text-4xl font-extrabold text-[#FF204E]">{totalVideos}</p>
        </div>
        {/* Card 4: Karya Unggulan */}
        <div className="p-6 rounded-2xl bg-[#5D0E41]/10 border border-white/5 space-y-2">
          <p className="text-xs font-semibold uppercase text-gray-400 tracking-wider">Karya Unggulan</p>
          <p className="text-3xl sm:text-4xl font-extrabold text-[#FF204E]">{totalFeatured}</p>
        </div>
      </div>

      {/* Tabel CRUD Portfolio */}
      <div className="rounded-2xl border border-white/5 bg-[#5D0E41]/5 overflow-hidden">
        {isLoading ? (
          <div className="p-20 text-center space-y-3">
            <div className="w-10 h-10 rounded-full border-4 border-[#FF204E]/20 border-t-[#FF204E] animate-spin mx-auto" />
            <p className="text-gray-400 text-sm">Memuat daftar portfolio...</p>
          </div>
        ) : items.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="bg-[#00224D] border-b border-white/10 text-gray-400 text-xs font-semibold uppercase tracking-wider">
                <tr>
              ...
              <tbody className="divide-y divide-white/5">
                {items.map((item) => {
                  const mediaDisplay = item.thumbnailUrls?.[0] || item.mediaUrls?.[0] || ''
                  return (
          ...
          <div className="p-20 text-center space-y-4">
            ...
            <h3 className="text-xl font-bold text-white">Belum Ada Portfolio</h3>
            <p className="text-gray-400 max-w-sm mx-auto text-sm">
              Tambahkan foto dan video perdana Anda untuk ditampilkan di galeri publik.
            </p>
          </div>
        )}
      </div>
